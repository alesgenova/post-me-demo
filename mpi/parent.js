...
function colorCombination(a, X, b, Y) {
  return [a * X[0] + b * Y[0], a * X[1] + b * Y[1], a * X[2] + b * Y[2]];
}
...
async function main(N) {
  ...
  const arrayImg = document.getElementById("array-img");
  const arrayResultSpan = document.getElementById("array-result-span"); // Pi section elements

  const dxSelect = document.getElementById("dx-select");
  dxSelect.value = "8";
  const calculateBtn = document.getElementById("calculate-btn");
  const piResultSpan = document.getElementById("pi-result-span");
  ...
  sortBtn.onclick = async function () {
    ...
    arrayResultSpan.textContent = "Sorted in ".concat(t1 - t0, "ms");
    renderArray(array, arrayImg, 2000);
    sortBtn.disabled = true;
  };
  ...
  calculateBtn.onclick = async function () {
    ...
    piResultSpan.textContent = "".concat(result, " in ").concat(t1 - t0, "ms");
  };
}
...
function renderArray(array, img, maxSize) {
  ...
  const colorMap = createColorMap(colors$5, scale);
  const stride = Math.floor(array.length / width);

  for (let i = 0; i < width; ++i) {
    colorMap(array[i * stride]).concat(1).map(v => v * 255).forEach((c, j) => {
