...
var model = {
  sum: function sum(x, y, onProgress) {
    return new Promise(function (resolve) {
      var nIterations = 10;

      var step = function step(iter) {
        if (iter >= nIterations) {
          resolve(x + y);
          return;
        }
...
var log = debug('post-me:worker');
messenger = DebugMessenger(messenger, log); // Start handshake with the parent

ChildHandshake(messenger, model).then(function (_connection) {});
