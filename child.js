...
var titleInput = document.getElementById("title-input"); // Create a Messenger to communicate with the parent window
...
ChildHandshake(messenger, model).then(function (connection) {
  ...
  remoteHandle.call('getTitle').then(function (title) {
    titleInput.value = title;
  });

  titleInput.oninput = function (ev) {
  ...
  remoteHandle.addEventListener('ping', function () {
    count += 1;
    countSpan.innerHTML = count.toString();
  });
});
